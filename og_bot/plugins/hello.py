...
@listen_to('^오늘의 점심은 (.*)$')
def write_today_lunch_menu(message, keyword):
    now = time.localtime()
    year = now.tm_year
    month = now.tm_mon
    today = now.tm_mday
    wday = WDAY[int(now.tm_wday)]
    text = str(year)+'년 '+str(month)+'월 '+str(today)+'일 '+wday+'의 점심은 '+keyword+' 입니다.'

    f = open('ate_lunch_menu.txt', 'r')
    wrote_text = f.readline()
