...
module.exports = (robot) ->
  robot.enter (res) ->
    ...
    if roomId is 'C1VQ8TBB5' # _general
      ...
      robot.messageRoom "#dev", "#{res.message.user.name} 님 반갑습니다! 이 곳은 개발에 관한 이야기를 하는 채널입니다."
    else if roomId is 'C3B4XEP1U' # dev_noti
      robot.messageRoom "#dev_noti", "#{res.message.user.name} 님 반갑습니다! 이 곳은 개발에 관련된 알림이 오는 채널입니다."
    else if roomId is 'C24PJ9KAR' # bot_test
      robot.messageRoom "#bot_test", "#{res.message.user.name} 님 반갑습니다! 이 곳은 슬랙봇을 테스트하는 채널입니다."
    else if roomId is 'C4DQ7DLKG' # ga
      robot.messageRoom "#ga", "#{res.message.user.name} 님 반갑습니다! 이 곳은 Google Analytics 에 대한 알림이 오는 채널입니다."
